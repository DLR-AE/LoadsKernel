...
#                      'Maximal time step number': 10, # for testing
    ...
    def tau_run(self):
        ...
        args_deform   = shlex.split('/opt/openmpi/bin/mpirun -n {} deformation para_subcase_{} ./log/log with mpi'.format(self.jcl.aero['tau_cores'], self.trimcase['subcase']))
        args_pre      = shlex.split('/opt/openmpi/bin/mpirun -n {} ptau3d.preprocessing para_subcase_{} ./log/log with mpi'.format(self.jcl.aero['tau_cores'], self.trimcase['subcase']))
        args_solve    = shlex.split('/opt/openmpi/bin/mpirun -n {} ptau3d.{} para_subcase_{} ./log/log with mpi'.format(self.jcl.aero['tau_cores'], self.jcl.aero['tau_solver'], self.trimcase['subcase']))
