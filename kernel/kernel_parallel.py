...
@author: voss_ar
"""
import cPickle, time, imp, sys, os, multiprocessing
import scipy
import numpy as np
...
def run_kernel(job_name, pre=False, main=False, post=False, test=False, path_input='../input/', path_output='../output/', jcl=None):
    ...
    if main:
        ...
        listener = pool.apply_async(mainprocessing_listener, (q, path_output, job_name, jcl)) # put listener to work

        # sending jobs to pool of workers
        jobs = []
        for i in range(len(jcl.trimcase)):
            job = pool.apply_async(mainprocessing_worker, (q, path_output, job_name, jcl, i))
            jobs.append(job)
         
        print 'All jobs launched, waiting for execution.'
        for job in jobs: 
            job.get()
        
        q.put('finish') # putting finish signal into queue for listener
        print 'All jobs finished, waiting for listener.'
        listener.get()
        pool.close() 
        print '--> Done in %.2f [sec].' % (time.time() - t_start)
...
def mainprocessing_worker(q, path_output, job_name, jcl, i):
    if not 'model' in locals():
        model = load_model(job_name, path_output)
    print ''
    print '========================================' 
    ...
    trim_i.response['i'] = i
    q.put(trim_i.response)
    del trim_i, post_processing_i
    return

def mainprocessing_listener(q, path_output, job_name, jcl):
    if not 'model' in locals():
        model = load_model(job_name, path_output)
