...
class jcl:
    def __init__(self):
        self.general = {# Give your aircraft a name
                        'aircraft':'DLR F-19-S',
        ...
        """
        self.efcs = {# Name of the corresponding python module
                     'version': 'mephisto',
                     # Path where to find the EFCS module
                     'path': '/path/to/EFCS',       
                    } 
        self.geom = {# Read the structural geometry from ModGen and/or Nastran (mona) BDFs
                     'method': 'mona',
                     ...
                     # bdf file with GRID-cards, one monitoring station is created at each GRID point, 1st GRID point -> 1st monstation
        ...
        self.aero = {# Settings for the aerodynamic model
        ...
        self.meshdefo = {# General CFD surface mesh information
                         'surface':
                                    {# implemented file formats: 'cgns', 'netcdf', 'su2'
                                     'fileformat': 'netcdf',
                                     # file name of the CFD mesh
                                     'filename_grid':'tau.grid',
                                     # list of markers [1, 2, ...] or ['upper', 'lower', ...] of surfaces to be included in deformation
                                     'markers': [1,3],
                                    },
                         # Volume mesh information, currently unused
                         'volume':{},
                        } 
        self.spline = {# Set the was in which the aerodynamic forces are applied to the structure. 
                       # Options: 'nearest_neighbour', 'rbf' or 'nastran'
        ...
        self.mass = {# Settings for the structural dynamics.
                     # Currently inplemented interfaces: 'f06', 'modalanalysis', 'guyan', 'CoFE', 'B2000' 
                     'method': 'modalanalysis', 
                     'key': ['M1', 'M2'],
        ...
        self.damping = {# Modal damping can be applied as a factor of the stiffness matrix.
                        'method': 'modal',
                        'damping': 0.02,
                       }
        self.atmo = {# The international standard atmosphere (ISA)
                     'method':'ISA', 
                     'key':['FL000','FL055', 'FL075', 'FL200', 'FL300', 'FL450'],
                     # Altitude in meters
                     'h': ft2m([0, 5500, 7500, 20000, 30000, 45000]), 
                    }
        self.eom = {# Setting of the rigid body equations of motion, 'linear' or 'waszak'
                    'version': 'linear'} 
        
        # --- Start of experimental section, only for special cases ---
        para_LG = { # Parameters for generic landing gear
                   # see PhD Thesis of Wolf Krueger and Sunpeth Cumnuantip
                   'stroke_length':  0.3,   # m
        ...
        self.landinggear = {# Activates a generic landing gear model during time simulation
                            'method': 'generic',
                            'key': ['MLG1', 'MLG2', 'NLG' ],
        ...
        self.engine= {# Activates an engine model: 'thrust_only', 'propellerdisk', 'PyPropMat' or 'VLM4Prop'
                      'method': 'thrust_only', 
        ...
        Because the data is calculated during the simulations, these sensors may be used as input for a flight controller or similar. 
        In case a wind sensor is specified here, this sensor is used to "measure" alpha and beta.
        """
        ...
        There may be hundreds or thousands of load cases, so at some point it might be beneficial to script this section or import an excel sheet.
        """
        self.trimcase = [{# Descriptive string of the maneuver case
                          # e.g. according to G. Pinho Chiozzotto, "Kriterien fuer die Erstellung eines Lastenkatalogs," Institute of 
                          # Aeroelasticity, iLOADs MS1.2, Feb. 2014.
                          'desc': 'CC.BFDM.OVCFL000.Vergleichsfall53', 
                          # Kind of trim condition, blank for trim about all three axes, for more trim conditions see trim_conditions.py
                          'maneuver': '',
        ...
        """
        self.simcase = [{# Time step size of the output in [s]
                         'dt': 0.01,
