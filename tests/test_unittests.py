...
def test_bdf_reader_with_includes(tmp_output):
    """
    ...
    """
    with open(os.path.join(tmp_output, 'parent.bdf'), 'w') as fid:
        fid.write("include './a_long_file_name_for_a_grid_point.bdf' \n")
        fid.write("include './one/ \n        more/ \n        grid.bdf' \n")
        fid.write('GRID           1       0     1.0     2.0     3.0       0 \n')

    with open(os.path.join(tmp_output, 'a_long_file_name_for_a_grid_point.bdf'), 'w') as fid:
        fid.write('GRID           2       0     1.0     2.0     3.0       0')

    os.makedirs(os.path.join(tmp_output, 'one', 'more'), exist_ok=True)
    with open(os.path.join(tmp_output, 'one', 'more', 'grid.bdf'), 'w') as fid:
        fid.write('GRID           3       0     1.0     2.0     3.0       0')

    # Read the BDFs created above.
    r = read_bdf.Reader()
    r.process_deck(os.path.join(tmp_output, 'parent.bdf'))
