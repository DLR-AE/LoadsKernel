...
class Efcs(HAP.Efcs):
    ...
    def fmu_init(self, filename, command_0, setpoint_v, setpoint_h, ):
        ...
        """
        Mail from Christian, 16.09.2020:
        %% CTRL SURFACE LIMITS
        surf_limit(1) =  -10*pi/180; % de_min
        surf_limit(2) =   10*pi/180; %  de_max
        surf_limit(3) =  -15*pi/180; % da_min
        surf_limit(4) =   15*pi/180; % da_max
        surf_limit(5) =  -15*pi/180; % dr_min
        surf_limit(6) =   15*pi/180; % dr_max
        """
        # apply limits to the FMU, careful with sign conventions!
        self.fmi.set_real([self.reference_values['surf_limit[1]'], self.reference_values['surf_limit[2]']], [-self.max_eta, -self.min_eta ])
        self.fmi.set_real([self.reference_values['surf_limit[3]'], self.reference_values['surf_limit[4]']], [ self.min_xi,   self.max_xi  ])
        self.fmi.set_real([self.reference_values['surf_limit[5]'], self.reference_values['surf_limit[6]']], [-self.max_zeta,-self.min_zeta])
    ...
    def controller(self, t, feedback):
        ...
        # Add the trim commands (unknown to the FMU), adjust sign conventions
        ...
        dcommand[0:4] = command_dxi, command_deta, command_dzeta, command_dthrust
