...
class PostProcessing(object):
    ...
    def force_summation_method(self):
        ...
        for i_step in range(len(response['t'])):
            if hasattr(self.jcl,'eom') and self.jcl.eom['version'] == 'waszak':
                ...
                d2Ug_dt2_r = self.PHIstrc_cg.dot( np.hstack((response['d2Ucg_dt2'][i_step,0:3] - response['g_cg'][i_step,:] - np.cross(response['dUcg_dt'][i_step,0:3], response['dUcg_dt'][i_step,3:6]), 
                                                        response['d2Ucg_dt2'][i_step,3:6] ))  ) 
            else:
                d2Ug_dt2_r = self.PHIstrc_cg.dot( np.hstack((response['d2Ucg_dt2'][i_step,0:3] - response['g_cg'][i_step,:], response['d2Ucg_dt2'][i_step,3:6])) ) # Nastran
    ...
    def euler_transformation(self):
        ...
        for i_step in range(len(response['t'])):
            ...
            coord_tmp['dircos'] = np.append(coord_tmp['dircos'], [self.PHIcg_norm[0:3,0:3].dot(calc_drehmatrix(response['X'][i_step,:][3], response['X'][i_step,:][4], response['X'][i_step,:][5])),
                                                                  np.eye(3)], axis=0)
