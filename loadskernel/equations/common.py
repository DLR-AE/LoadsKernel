...
class Common():
    ...
    def cs_nonlin(self, dUcg_dt, X, Ux2, Vtas):
        ...
        for i_x2 in range(len(self.efcs.keys)):
            ...
            wj += self.x2grid['eff'][i_x2] * np.sign(Ux2[i_x2]) * np.sqrt(np.sin(Ujx2[self.aerogrid['set_j'][:,4]])**2.0 + \
    ...
    def cs(self, X, Ux2, q_dyn):
        ...
        for i_x2 in range(len(self.efcs.keys)):
            ...
            if self.hingeline == 'y':
                ...
                Ujx2 = np.dot(self.Djx2[i_x2],[0,0,0,0,0,Ux2[i_x2]])
            wj += self.x2grid['eff'][i_x2] * np.sum(self.aerogrid['N'][:] * np.cross(Ujx2[self.aerogrid['set_j'][:,(3,4,5)]], np.array([-1.,0.,0.])),axis=1)
        Pk = self.calc_Pk(q_dyn, wj)
        return Pk, wj
    ...
    def flexible_nonlin(self, dUcg_dt, Uf, dUf_dt, Vtas):
        if 'flex' in self.jcl.aero and self.jcl.aero['flex']:
            ...
            wjf_1 = np.sum(self.aerogrid['N'][:] * np.cross(Ujf[self.aerogrid['set_j'][:,(3,4,5)]], dUmac_dt[0:3]),axis=1) * -1 
            # modale Bewegung
    ...
    def flexible(self, Uf, dUf_dt, dUcg_dt, q_dyn, Vtas):
        ...
        if 'flex' in self.jcl.aero and self.jcl.aero['flex']:
            ...
            # modale Verformung
            Ujf = np.dot(self.PHIjf, Uf )
            wjf_1 = np.sum(self.aerogrid['N'][:] * np.cross(Ujf[self.aerogrid['set_j'][:,(3,4,5)]], dUmac_dt[0:3]),axis=1) / Vtas
            # modale Bewegung
            # zu Ueberpruefen, da Trim bisher in statischer Ruhelage und somit  dUf_dt = 0
            dUjf_dt = np.dot(self.PHIjf, dUf_dt ) # viel schneller!
    ...
    def gust(self, X, q_dyn):
        ...
        if self.is_sim and 'gust' in self.simcase and self.simcase['gust']:
            ...
            gust_direction_vector = np.sum(self.aerogrid['N'] * np.dot(np.array([0,0,1]), calc_drehmatrix( self.simcase['gust_orientation']/180.0*np.pi, 0.0, 0.0 )), axis=1)
    ...
    def get_sensor_onflow(self, i_sensor, X, Vtas, Uf, dUf_dt):
        ...
        uf_1, vf_1, wf_1 = np.cross(self.PHIf_sensor.T.dot(Uf)[self.sensorgrid['set'][i_sensor,(3,4,5)]], X[6:9]).dot(self.PHIcg_norm[:3,:3])
        # additional wind from flexible velocity
        uf_2, vf_2, wf_2 = self.PHIf_sensor.T.dot(dUf_dt)[self.sensorgrid['set'][i_sensor,0:3]].dot(self.PHIcg_norm[:3,:3])
...
#             # modale Bewegung
#             dUjf_dt = np.dot(self.PHIjf, dUf_dt ) # viel schneller!
#             wjf_2 = np.sum(self.aerogrid['N'][:] * dUjf_dt[self.aerogrid['set_j'][:,(0,1,2)]],axis=1) / Vtas * -1.0
#             wjf = wjf_1 + wjf_2
    ...
    def landinggear(self, X, Tbody2geo):
        ...
        if self.is_sim and 'landinggear' in self.simcase and self.simcase['landinggear']:
            ...
            if self.jcl.landinggear['method'] in ['generic']:
                p2  = X[self.solution.idx_lg_states[:self.extragrid['n']]]  # position Tire center over ground
                dp2 = X[self.solution.idx_lg_states[self.extragrid['n']:]] # velocity Tire center
